...
const disconnectionTimers = new Map<number, NodeJS.Timeout>();
        ...
        if (room) {
          ...
          if (gameState) {
            ...
            const timer = setTimeout(async () => {
              ...
              const currentGameState = await getGameState(room.id);
              const currentRoom = await getRoomById(room.id);
...
async function handleGameEvent(event: IncomingGameMessage): Promise<void> {
  ...
  switch (type) {
    ...
    case 'PLAYER_SURRENDER':
      try {
        const surrenderPayload = payload as any;
...
async function handlePlayerSurrender(
  matchId: string,
  ...
  try {
    ...
    await updatePlayerElo(winnerId, eloWinner);
    await updatePlayerElo(loserId, eloLoser);
    
    // Finalizar la sala
    await finishGameRoom(matchId);
